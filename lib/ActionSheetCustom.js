...
class ActionSheet extends React.Component {
  ...
  _createButton(title, index) {
    ...
    const fontWeight = destructiveButtonIndex === index ? '900' : '500';

    // console.log('title', this.props.title);
    const firstButton = index === 1 ? true : false;
    const lastButton = index === options.length -1 ? true : false;
    let buttonBoxStyleSelect = styles.buttonBox;
    
    if (firstButton && this.props.title === undefined) {
      buttonBoxStyleSelect = styles.firstButtonBox;
    }  
    
    if (lastButton) {
      buttonBoxStyleSelect = styles.lastButtonBox;
    }

    if (options.length === 2) {
      buttonBoxStyleSelect = styles.singleButtonBox;
    }
    
    const buttonBoxStyle =
      cancelButtonIndex === index ? styles.cancelButtonBox : buttonBoxStyleSelect;
    return (
      <TouchableHighlight
        key={index}
        activeOpacity={1}
      ...
      >
        {React.isValidElement(title) ? (
          ...
          <Text style={[styles.buttonText, { color: fontColor, fontWeight }]}>{title}</Text>
        )}
      </TouchableHighlight>
    );
  }
  ...
  _renderOptions() {
    ...
    return this.props.options.map((title, index) =>
      cancelButtonIndex === index ? null : this._createButton(title, index)
    );
  }

  render() {
    const styles = this.styles;
    const { visible, sheetAnim, opacityAnim } = this.state;
    return (
      <Modal
        visible={visible}
        animationType="none"
        ...
        <View style={[styles.wrapper]}>
          ...
          >
            ...
            <ScrollView
              ...
              showsVerticalScrollIndicator={this.scrollEnabled}
            >
              {this._renderOptions()}
            </ScrollView>
